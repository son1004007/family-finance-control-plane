...
SOURCE_TYPE="${GENERIC_IMPORT_SOURCE_TYPE:-generic_csv}"
ORIGINAL_FILENAME="${GENERIC_IMPORT_ORIGINAL_FILENAME:-$(basename "$CSV_INPUT")}"
...
case "$SOURCE_TYPE" in ''|*[!A-Za-z0-9_]*) echo 'Invalid source type' >&2; exit 3 ;; esac

SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd -P)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
IMPORTER_DIR="$REPO_ROOT/importers/generic_csv"
[ -f "$IMPORTER_DIR/emit_copy_sql_v2.py" ] || { echo 'generic importer v2 not found' >&2; exit 4; }
...
FILE_SHA="${GENERIC_IMPORT_FILE_SHA:-$(sha256_file "$CSV_FILE")}"
MAPPING_SHA="$(sha256_file "$MAPPING_FILE")"
case "$FILE_SHA$MAPPING_SHA" in *[!0-9a-fA-F]*) echo 'Invalid SHA-256 output' >&2; exit 8 ;; esac
[ "${#FILE_SHA}" -eq 64 ] && [ "${#MAPPING_SHA}" -eq 64 ] || { echo 'Invalid SHA-256 length' >&2; exit 8; }
...
if printf '%s\n' "$OUTPUT" | grep -q '^IMPORT_BATCH=FAIL '; then
  echo "GENERIC_IMPORT_RECONCILIATION=FAIL source_type=$SOURCE_TYPE file_sha256=$FILE_SHA" >&2
  exit 10
fi
